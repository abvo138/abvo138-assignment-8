...
def calculate_log_loss(y_true, y_prob):
    """
    Calculate binary log loss manually.
    ...
    y_prob = np.clip(y_prob, epsilon, 1 - epsilon)  # Clip probabilities for numerical stability
...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        loss = calculate_log_loss(y, y_prob[:, 1])
        ...
        plt.plot(x_vals, y_vals, color = 'black', linestyle = '--', label = "Decsion Boundary")
        plt.legend
